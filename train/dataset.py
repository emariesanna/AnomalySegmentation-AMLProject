...
def get_cityscapes_loader(datadir, batch_size=1, subset="val", num_workers=4, imagesize=(512, 1024)):

    input_transform = Compose([Resize(imagesize, Image.BILINEAR), ToTensor()])
    target_transform = Compose([Resize(imagesize, Image.NEAREST), ToLabel(), Relabel(255, 19)]) #transform label 255 (ignore label) to 19

    return DataLoader(cityscapes(datadir, input_transform, target_transform, subset=subset), num_workers=num_workers, batch_size=batch_size, shuffle=False)
...
class cityscapes(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.input_transform is not None:
            image ,label = self.input_transform(image, label)
